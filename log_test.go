...
func TestBasic(t *testing.T) {
	...
	t.Run("EmptyRelative", func(t *testing.T) {
		coff, cmsgs, err = l.Consume(OffsetOldest, &ConsumeOptions{MaxMessages: 10})
		require.NoError(t, err)
		require.Equal(t, int64(0), coff)
		require.Nil(t, cmsgs)

		coff, cmsgs, err = l.Consume(OffsetNewest, &ConsumeOptions{MaxMessages: 10})
		require.NoError(t, err)
		require.Equal(t, int64(0), coff)
		require.Nil(t, cmsgs)
	})
	...
	t.Run("EmptyAbsolute", func(t *testing.T) {
		coff, cmsgs, err = l.Consume(0, &ConsumeOptions{MaxMessages: 10})
		require.NoError(t, err)
		require.Equal(t, int64(0), coff)
		require.Nil(t, cmsgs)

		coff, cmsgs, err = l.Consume(1, &ConsumeOptions{MaxMessages: 10})
	...
	t.Run("Absolute", func(t *testing.T) {
		...
		coff, cmsgs, err = l.Consume(2, &ConsumeOptions{MaxMessages: 10})
		require.NoError(t, err)
		require.Equal(t, int64(3), coff)
		require.Equal(t, msgs[2:3], cmsgs)

		coff, cmsgs, err = l.Consume(3, &ConsumeOptions{MaxMessages: 10})
		require.NoError(t, err)
		require.Equal(t, int64(3), coff)
		require.Nil(t, cmsgs)

		coff, cmsgs, err = l.Consume(4, &ConsumeOptions{MaxMessages: 10})
		require.ErrorIs(t, err, ErrInvalidOffset)
		require.Equal(t, OffsetInvalid, coff)
		require.Nil(t, cmsgs)
	})
	...
	t.Run("RolloverAbsolute", func(t *testing.T) {
		coff, cmsgs, err = l.Consume(0, &ConsumeOptions{MaxMessages: 3})
		require.NoError(t, err)
		require.Equal(t, int64(3), coff)
		require.Equal(t, cmsgs, msgs[0:3])

		coff, cmsgs, err = l.Consume(coff, &ConsumeOptions{MaxMessages: 3})
		require.NoError(t, err)
		require.Equal(t, int64(6), coff)
		require.Equal(t, cmsgs, msgs[3:])

		coff, cmsgs, err = l.Consume(coff, &ConsumeOptions{MaxMessages: 10})
		require.NoError(t, err)
		require.Equal(t, int64(6), coff)
		require.Nil(t, cmsgs)

		coff, cmsgs, err = l.Consume(coff+1, &ConsumeOptions{MaxMessages: 10})
		require.ErrorIs(t, err, ErrInvalidOffset)
		require.Equal(t, OffsetInvalid, coff)
		require.Nil(t, cmsgs)
	})
...
func TestByKey(t *testing.T) {
	t.Run("NoIndex", func(t *testing.T) {
		...
		coff, cmsgs, err := l.ConsumeByKey([]byte("key"), OffsetOldest, &ConsumeOptions{MaxMessages: 32})
		require.ErrorIs(t, err, ErrNoIndex)
		require.Equal(t, OffsetInvalid, coff)
		require.Nil(t, cmsgs)
	})
	...
	t.Run("Empty", func(t *testing.T) {
		...
		coff, cmsgs, err := l.ConsumeByKey(msgs[0].Key, OffsetOldest, &ConsumeOptions{MaxMessages: 32})
		require.NoError(t, err)
		require.Equal(t, int64(0), coff)
		require.Nil(t, cmsgs)
	})
	...
	t.Run("Put", func(t *testing.T) {
		...
		for i, msg := range msgs {
			...
			coff, cmsgs, err := l.ConsumeByKey(msg.Key, OffsetOldest, &ConsumeOptions{MaxMessages: 32})
			require.NoError(t, err)
			...
			coff, cmsgs, err = l.ConsumeByKey(msg.Key, coff, &ConsumeOptions{MaxMessages: 32})
			require.NoError(t, err)
	...
	t.Run("After", func(t *testing.T) {
		coff, cmsgs, err := l.ConsumeByKey(msgs[0].Key, msgs[1].Offset, &ConsumeOptions{MaxMessages: 32})
		require.NoError(t, err)
		require.Equal(t, int64(4), coff)
		require.Nil(t, cmsgs)
	})
	...
	t.Run("Update", func(t *testing.T) {
		...
		for i := range msgs {
			...
			coff, cmsgs, err := l.ConsumeByKey(msgs[i].Key, OffsetOldest, &ConsumeOptions{MaxMessages: 32})
			require.NoError(t, err)
			require.Equal(t, int64(i+1), coff)
			require.Len(t, cmsgs, 1)
			require.Equal(t, msgs[i], cmsgs[0])

			coff, cmsgs, err = l.ConsumeByKey(msgs[i].Key, coff, &ConsumeOptions{MaxMessages: 32})
			require.NoError(t, err)
...
func testReopenSegment(t *testing.T) {
	...
	coff, cmsgs, err := l.Consume(OffsetOldest, &ConsumeOptions{MaxMessages: 4})
	require.NoError(t, err)
...
func testReopenSegments(t *testing.T) {
	...
	coff, cmsgs, err = l.Consume(coff, &ConsumeOptions{MaxMessages: 4})
	require.NoError(t, err)
	require.Equal(t, int64(4), coff)
...
func testReadonlyEmpty(t *testing.T) {
	...
	// Consume checks
	coff, cmsgs, err := l.Consume(OffsetOldest, &ConsumeOptions{MaxMessages: 1})
	require.NoError(t, err)
	require.Equal(t, int64(0), coff)
	require.Empty(t, cmsgs)

	coff, cmsgs, err = l.Consume(OffsetNewest, &ConsumeOptions{MaxMessages: 1})
	require.NoError(t, err)
	require.Equal(t, int64(0), coff)
	require.Empty(t, cmsgs)

	coff, cmsgs, err = l.Consume(0, &ConsumeOptions{MaxMessages: 1})
	require.NoError(t, err)
	require.Equal(t, int64(0), coff)
	require.Empty(t, cmsgs)

	coff, cmsgs, err = l.Consume(1, &ConsumeOptions{MaxMessages: 1})
	require.ErrorIs(t, err, ErrInvalidOffset)
	require.Equal(t, OffsetInvalid, coff)
	require.Empty(t, cmsgs)
...
func testReadonlySegment(t *testing.T) {
	...
	coff, cmsgs, err = l.Consume(OffsetNewest, &ConsumeOptions{MaxMessages: 4})
	require.NoError(t, err)
	require.Equal(t, int64(4), coff)
	require.Empty(t, cmsgs)
	...
	coff, cmsgs, err = l.Consume(4, &ConsumeOptions{MaxMessages: 4})
	require.NoError(t, err)
	require.Equal(t, int64(4), coff)
	require.Empty(t, cmsgs)

	coff, cmsgs, err = l.Consume(5, &ConsumeOptions{MaxMessages: 1})
	require.ErrorIs(t, err, ErrInvalidOffset)
	require.Equal(t, OffsetInvalid, coff)
	require.Empty(t, cmsgs)
...
func testReadonlySegments(t *testing.T) {
	...
	coff, cmsgs, err = l.Consume(2, &ConsumeOptions{MaxMessages: 4})
	require.NoError(t, err)
	require.Equal(t, int64(4), coff)
	require.Equal(t, msgs[2:], cmsgs)
...
func testBackupSegment(t *testing.T) {
	...
	for {
		coff, cmsgs, err := l.Consume(startOffset, &ConsumeOptions{MaxMessages: 1})
		require.NoError(t, err)

		boff, bmsgs, err := bl.Consume(startOffset, &ConsumeOptions{MaxMessages: 1})
		require.NoError(t, err)
...
func TestReindex(t *testing.T) {
	...
	coff, cmsgs, err := l.Consume(message.OffsetOldest, &ConsumeOptions{MaxMessages: 32})
	require.NoError(t, err)
	require.Equal(t, int64(2), coff)
	require.Equal(t, msgs[0:2], cmsgs)

	coff, cmsgs, err = l.Consume(coff, &ConsumeOptions{MaxMessages: 32})
	require.NoError(t, err)
	require.Equal(t, int64(4), coff)
	require.Equal(t, msgs[2:], cmsgs)
...
func testDeleteReaderPartial(t *testing.T) {
	...
	doff, dmsgs, err := l.Consume(message.OffsetOldest, &ConsumeOptions{MaxMessages: 32})
	require.NoError(t, err)
...
func testDeleteWriterSingle(t *testing.T) {
	...
	doff, dmsgs, err = l.Consume(1, &ConsumeOptions{MaxMessages: 32})
	require.NoError(t, err)
	require.Equal(t, int64(4), doff)
	require.Equal(t, msgs[2:4], dmsgs)
}
...
func testDeleteWriterPartial(t *testing.T) {
	...
	doff, dmsgs, err = l.Consume(2, &ConsumeOptions{MaxMessages: 32})
	require.NoError(t, err)
	require.Equal(t, int64(4), doff)
...
func testDeleteAll(t *testing.T) {
	...
	coff, cmsgs, err = l.Consume(1, &ConsumeOptions{MaxMessages: 32})
	require.NoError(t, err)
	require.Equal(t, int64(4), coff)
	require.Empty(t, cmsgs)
...
func testConcurrentPubsubRecent(t *testing.T) {
	...
	g.Go(func() error {
		...
		for ctx.Err() == nil {
			next, msgs, err := s.Consume(offset, &ConsumeOptions{MaxMessages: 32})
			if err != nil {
	...
	g.Go(func() error {
		for ctx.Err() == nil {
			_, msgs, err := s.Consume(OffsetOldest, &ConsumeOptions{MaxMessages: 32})
			if err != nil {
				return err
			}
...
func testConcurrentConsume(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		go func() {
			...
			offset := OffsetOldest
			for offset < 30000 {
				next, _, err := s.Consume(offset, &ConsumeOptions{MaxMessages: 1})
				require.NoError(t, err)
				offset = next
			}
		}()
...
func testConcurrentDelete(t *testing.T) {
	...
	go func() {
		...
		for !t.Failed() {
			next, msgs, err := s.Consume(OffsetOldest, &ConsumeOptions{MaxMessages: 10})
			require.NoError(t, err)
...
func testConcurrentGC(t *testing.T) {
	...
	g.Go(func() error {
		for ctx.Err() == nil {
			next, consumed, err := l.Consume(OffsetOldest, &ConsumeOptions{MaxMessages: 32})
			if err != nil {
				return err
			}
